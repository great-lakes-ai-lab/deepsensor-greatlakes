...
train_range = ("1992-01-01", "2021-12-31")
...
# Create an 'index' dimension for your points
points_idx = np.arange(random_lake_points.shape[0])

# Convert the numpy array to an xarray Dataset
# 'lat' and 'lon' are common variable names for coordinates in DeepSensor
random_lake_points_xr = xr.Dataset(
    {
        'lat': (('index'), random_lake_points[:, 0]), # Assuming 1st column is latitude
        'lon': (('index'), random_lake_points[:, 1])  # Assuming 2nd column is longitude
    },
    coords={'index': points_idx}
)

# Now save the xarray Dataset
fixed_context_points_filename = 'fixed_training_context_points_{N}.nc'
random_lake_points_xr.to_netcdf(fixed_context_points_filename)
print(f"Saved fixed training context points to: {fixed_context_points_filename}")
sys.stdout.flush()
...
for epoch in tqdm(range(50), desc="Training Epochs"): # Training for selected number of epochs
...
# Extract the date from test_task once
prediction_date_str = str(test_task.date)[:10] # e.g., '2010-02-14'

plt.subplot(1, 2, 1)
prediction_ds_masked['mean'].plot(cmap='viridis', cbar_kwargs={'label': 'Predicted Mean SST'})
plt.title(f'Masked Predicted Mean SST for {prediction_date_str}')
plt.xlabel('Longitude')
plt.ylabel('Latitude')

plt.subplot(1, 2, 2)
prediction_ds_masked['std'].plot(cmap='plasma', cbar_kwargs={'label': 'Predicted Std SST'})
plt.title(f'Masked Predicted Std SST for {prediction_date_str}')
plt.xlabel('Longitude')
plt.ylabel('Latitude')

plt.tight_layout()
prediction_map_filename = os.path.join(PLOT_OUTPUT_DIR, f'masked_prediction_maps_{prediction_date_str}.png')
